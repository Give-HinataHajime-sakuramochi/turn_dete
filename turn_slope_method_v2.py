...
def Cal_SV(now_s,front_s,isallzero): #只有一條或沒偵測到線的時候用
    string = ""
    if isallzero==1:
        string="0"
    elif now_s-front_s<0:
        string="turn left"
    elif now_s-front_s>0:
        string = "turn right"
    else:
...
while cap.isOpened():
    ...
    if (i!=0 and i%5==0) or i==1:
        if left_s!=0 and right_s!=0:  #有兩條線
            ...
            if left_s>0:
                turn_dir = Cal_SV(left_s,front_f_ls,0)
                front_f_ls = left_s
            elif right_s>0:
                turn_dir = Cal_SV(right_s, front_f_ls,0)
                front_f_ls = right_s
            elif left_s<0:
                turn_dir = Cal_SV(left_s, front_f_rs, 0)
                front_f_rs = left_s
            elif right_s<0:
                turn_dir = Cal_SV(right_s, front_f_rs,0)
                front_f_rs = right_s
            else:
                turn_dir = Cal_SV(0, 0, 1)
        if turn_dir=="0":
            turn_dir=last_dir
    ...
    cv2.circle(img, (int(point_x), int(point_y)), 10, (0, 0, 255), cv2.FILLED)
    cv2.circle(img, (int(point_x), int(point_y / 2)), 10, (0, 0, 255), cv2.FILLED)
    cv2.circle(img, (int(intersection_x), int(intersection_y)), 10, (0, 0, 255), cv2.FILLED)
    cv2.line(img, (int(point_x), int(point_y)), (int(point_x), int(point_y / 2)), (0, 255, 255), 5)
    if intersection_x!=0 and intersection_y!=0:
        cv2.line(img, (int(point_x), int(point_y)), (int(intersection_x), int(intersection_y)), (0, 255, 255), 5)
    out.write(img)
